 ...
 */
 ...
 /* GM program numbers (0-based) */
 #define GM_PIANO   0
 #define GM_GUITAR  27   /* Electric Guitar (clean) */
 ...
 typedef enum { WORKER_DRUMS = 0, WORKER_GUITAR = 1, WORKER_PIANO = 2 } WorkerRole;
 ...
 typedef struct {
     ...
     int        genre;     /* 0=pop, 1=jazz, 2=blues */
     WorkerRole role;
 } SongSpec;
 ...
 static void db_init(DynBuf *db) {
     ...
     if (!db->data) { perror("malloc DynBuf"); exit(EXIT_FAILURE); }
 }
 
 static void db_push(DynBuf *db, const uint8_t *bytes, size_t n) {
     if (db->len + n > db->cap) {
         while (db->len + n > db->cap) db->cap *= 2;
         uint8_t *tmp = realloc(db->data, db->cap);
         if (!tmp) { perror("realloc DynBuf"); free(db->data); exit(EXIT_FAILURE); }
         db->data = tmp;
     }
 ...
 static void db_push_vlq(DynBuf *db, uint32_t v) {
     ...
     tmp[len++] = v & 0x7F;
     v >>= 7;
     while (v) { tmp[len++] = (v & 0x7F) | 0x80; v >>= 7; }
     /* tmp is LE; write BE */
     for (int i = len - 1; i >= 0; i--) db_push1(db, tmp[i]);
 }
 ...
 static uint8_t *make_mtrk(const uint8_t *events, size_t ev_len,
     ...
     if (!chunk) { perror("malloc mtrk"); exit(EXIT_FAILURE); }
 
     chunk[0] = 'M'; chunk[1] = 'T'; chunk[2] = 'r'; chunk[3] = 'k';
 ...
 /* I–V–vi–IV progressions per genre (semitones from root) */
 static int bar_root(int key_root, int bar, int genre) {
     static const int pop[]  = {0, 7, 9, 5};
     static const int jazz[] = {0, 5, 7, 2};
     static const int blues[]= {0, 0, 5, 5};
     const int *prog = (genre == 1) ? jazz : (genre == 2) ? blues : pop;
  ...
  *   36 = Bass Drum, 38 = Snare, 42 = Closed HH, 46 = Open HH, 51 = Ride
  *
  * Patterns:
  *   Pop  – kick 1&3, snare 2&4, closed HH every 8th note
  *   Jazz – kick beat 1, snare 2&4, ride every beat
  *   Blues– kick 1&3, snare 2&4, open HH on up-beats
  *
 ...
 typedef struct { uint32_t tick; int is_on; uint8_t note; uint8_t vel; } DrumEv;
 
 static int drum_ev_cmp(const void *a, const void *b) {
     const DrumEv *da = (const DrumEv *)a;
     const DrumEv *db = (const DrumEv *)b;
     if (da->tick != db->tick) return (da->tick < db->tick) ? -1 : 1;
 ...
 static void gen_drums(const SongSpec *s, DynBuf *db) {
     ...
     if (!evs) { perror("malloc drum events"); exit(EXIT_FAILURE); }
     int nev = 0;
     ...
     for (int i = 0; i < nev; i++) {
         ...
         cur = evs[i].tick;
         if (evs[i].is_on) emit_on (db, delta, CH, evs[i].note, evs[i].vel);
         else               emit_off(db, delta, CH, evs[i].note);
     }
     ...
     for (int bar = 0; bar < s->bars; bar++) {
         ...
         for (int beat = 0; beat < s->beats_per_bar; beat++) {
             ...
             for (int n = 0; n < 3 && nev + 2 <= MAX_GUITAR_EVENTS; n++) {
                 /* strum spread: 10 ticks per string */
                 evs[nev++] = (GuitarEv){ bt  + (uint32_t)n * 10, 1, (uint8_t)notes[n], vel };
                 evs[nev++] = (GuitarEv){ off + (uint32_t)n * 5,  0, (uint8_t)notes[n], 0   };
             }
         }
     }
 ...
 typedef struct { uint32_t tick; int is_on; uint8_t note; uint8_t vel; } PianoEv;
 
 static int piano_ev_cmp(const void *a, const void *b) {
     const PianoEv *pa = (const PianoEv *)a;
     const PianoEv *pb = (const PianoEv *)b;
     if (pa->tick != pb->tick) return (pa->tick < pb->tick) ? -1 : 1;
 ...
 static void gen_piano(const SongSpec *s, DynBuf *db) {
     ...
     const int *scale     = (s->genre == 2) ? minor_penta : major_scale;
     int        scale_len = (s->genre == 2) ? 5           : 7;
 
     const uint32_t EIGHT = TPQ / 2;
     const uint8_t  CH    = CH_PIANO;
     int si = 0, dir = 1;
 
     emit_prog(db, CH, GM_PIANO);
 
     PianoEv *evs = malloc(MAX_PIANO_EVENTS * sizeof(PianoEv));
     if (!evs) { perror("malloc piano events"); exit(EXIT_FAILURE); }
     int nev = 0;
 ...
 static void child_main(int spec_fd, int track_fd, WorkerRole role) {
     const char *name = (role == WORKER_DRUMS)  ? "Drums"
                      : (role == WORKER_GUITAR) ? "Guitar" : "Piano";
     ...
     if (close(spec_fd) < 0) { perror("child close spec_fd"); exit(EXIT_FAILURE); }
     ...
     db_init(&events);
     switch (role) {
         case WORKER_DRUMS:  gen_drums (&spec, &events); break;
         case WORKER_GUITAR: gen_guitar(&spec, &events); break;
         case WORKER_PIANO:  gen_piano (&spec, &events); break;
     }
     ...
     if (close(track_fd) < 0) { perror("child close track_fd"); exit(EXIT_FAILURE); }
...
static void write_midi_file(const char *path, int bpm,
     ...
     if (fwrite(mthd, 1, sizeof(mthd), f) != sizeof(mthd)) {
         perror("fwrite MThd"); fclose(f); exit(EXIT_FAILURE);
     }
     ...
     if (fwrite(mtrk_hdr,     1, 8,                 f) != 8 ||
         fwrite(tempo_events, 1, sizeof(tempo_events), f)
               != sizeof(tempo_events)) {
         perror("fwrite tempo track"); fclose(f); exit(EXIT_FAILURE);
     }
...
static int read_int(const char *prompt, int lo, int hi) {
 ...
 int main(void) {
     ...
     printf("Genre  0=Pop  1=Jazz  2=Blues\n");
     int genre = read_int("Genre", 0, 2);
     printf("Key root MIDI note  (C4=60  D4=62  E4=64  F4=65  G4=67  A4=69  B4=71)\n");
     int key_root = read_int("Key root", 48, 84);
 
     printf("\n[Parent] Parameters: BPM=%d  bars=%d  genre=%d  key_root=%d\n\n",
            bpm, bars, genre, key_root);
     fflush(stdout);
     ...
     for (int i = 0; i < NUM_WORKERS; i++) {
         if (pipe(to_child[i])   < 0) { perror("pipe to_child");   exit(EXIT_FAILURE); }
         if (pipe(from_child[i]) < 0) { perror("pipe from_child"); exit(EXIT_FAILURE); }
     }
     ...
     for (int i = 0; i < NUM_WORKERS; i++) {
         ...
         if (pid == 0) {
             ...
             for (int j = 0; j < NUM_WORKERS; j++) {
                 ...
                 if (close(to_child[j][1]) < 0)   { perror("child close"); exit(EXIT_FAILURE); }
                 /* Parent side of from_child: child never reads */
                 if (close(from_child[j][0]) < 0) { perror("child close"); exit(EXIT_FAILURE); }
                 /* Sibling pipes: not our business */
                 if (j != i) {
                     if (close(to_child[j][0])   < 0) { perror("child close"); exit(EXIT_FAILURE); }
                     if (close(from_child[j][1]) < 0) { perror("child close"); exit(EXIT_FAILURE); }
                 }
             }
         ...
         printf("[Parent] Forked %s worker (PID %d)\n",
              ...
              : (roles[i] == WORKER_GUITAR)? "Guitar" : "Piano", pid);
         fflush(stdout);
     }
 
     /* ── Parent: close child-side pipe ends ──────────────────────────────── */
     for (int i = 0; i < NUM_WORKERS; i++) {
         if (close(to_child[i][0])   < 0) { perror("parent close to_child[0]");   exit(EXIT_FAILURE); }
         if (close(from_child[i][1]) < 0) { perror("parent close from_child[1]"); exit(EXIT_FAILURE); }
     }
     ...
     for (int i = 0; i < NUM_WORKERS; i++) {
         ...
         if (close(to_child[i][1]) < 0) { perror("parent close to_child[1]"); exit(EXIT_FAILURE); }
     ...
     for (int i = 0; i < NUM_WORKERS; i++) {
         ...
         if (!tracks[i].data) { perror("malloc tracks[i]"); exit(EXIT_FAILURE); }
         tracks[i].len  = len32;
         safe_read_exact(from_child[i][0], tracks[i].data, len32, "parent←child data");
 
         if (close(from_child[i][0]) < 0) {
             perror("parent close from_child[0]"); exit(EXIT_FAILURE);
         }
         printf("[Parent] Received %u bytes from %s worker\n",
                len32,
                (roles[i] == WORKER_DRUMS)  ? "Drums"
              : (roles[i] == WORKER_GUITAR) ? "Guitar" : "Piano");
         fflush(stdout);
     }
     ...
     for (int i = 0; i < NUM_WORKERS; i++) {
         int status;
         if (waitpid(pids[i], &status, 0) < 0) { perror("waitpid"); exit(EXIT_FAILURE); }
         if (WIFEXITED(status))
